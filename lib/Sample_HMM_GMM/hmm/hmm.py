...
class HMM():
    ...
    def viterbi(self, observation):
        # record viterbi history
        history = {'delta':[], 'bp_matrix':[], 'state_q':None, 'prob_log': None}
        ...
        for t in range(obs_length):
            ...
            # record delta
            delta_list = delta_log.tolist()
            for row in range(self.states.length):
                for col in range(self.states.length):
                    if delta_list[row][col] == -float('inf'):
                        delta_list[row][col] = '-Infinity'
            
            history['delta'].append(delta_list)
        ...
        history['state_q']   = state_q.tolist()
        history['bp_matrix'] = bp_matrix.tolist()
        history['prob_log']  = prob_log
        return prob_log, state_q, history
    ...
    def train(self, observations, epoch):
        ...
        # init
        history['init'] = {
            'sequences'  : [((np.concatenate(seq, axis=0)).squeeze()).tolist() for seq in states_sequences],
            'emissions'  : [
                {
                    'means'      : gmm.means_.tolist(),
                    'weights'    : gmm.weights_.tolist(),
                    'covariances': gmm.covariances_.tolist()
                } for gmm in self.states.emissions
            ],
            'transitions' : self.states.transition_matrix.tolist(),
            'viterbi': None,
            'decode_log_prob': None
        }
        ...
        for e in range(epoch):
            ...
            states_sequences = [[] for s in range(self.states.length)]
            for observation in observations:
                decode_prob, decode_states, vhistory = self.viterbi(observation)
            ...
            decode_log_prob /= len(observations)
        ...
        return decode_log_prob, history